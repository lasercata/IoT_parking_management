...
\begin{document}
    ...
    \begin{indt}{\section{Design}} %{{{1
        ...
        \begin{indt}{\subsection{Node Automaton}} %{{{2
            ...
            Figure~\ref{fig:node_fsm} illustrates the complete state machine implemented on the node.
            ...
            The following table summarizes the main hardware components and software technologies used to implement the node and the associated platform.
            ...
            \end{table}% }}}3
             ...
             Finally, Figure~\ref{fig:IoT_node_schematic} presents the wiring diagram of the node, detailing the interconnections between the microcontroller, sensors, RFID module and LED. This schematic reflects the current prototype implementation.
...
\begin{indt}{\section{Digital Twin VS. Real object}} %{{{2
    %TODO
\end{indt}

\begin{indt}{\section{Ways of Improvement}} %{{{1

    \begin{indt}{\subsection{Improved Authentication Mechanism for the Node}}
        Currently, the system uses a token-based authentication approach with tokens hardcoded directly into the node firmware. This method presents security risks, as compromised firmware exposes valid credentials. A more secure solution would involve dynamic token provisioning, certificate-based authentication.
    \end{indt}

    \begin{indt}{\subsection{Automated Deployment of New Nodes}}
        At present, nodes are deployed manually, requiring firmware flashing, token generation, ID assignment, and database registration. This process could be automated through a provisioning script or pipeline capable of:
        \begin{itemize}
    ...
    \begin{indt}{\subsection{Design of a Dedicated PCB}}
        The current prototype relies on development boards and external wiring. A dedicated PCB integrating all required sensors and communication modules would improve reliability, reduce power consumption, minimize wiring issues, and make the system more suitable for large-scale deployment.
    \end{indt}

    \begin{indt}{\subsection{Power Management Optimization}}
        The system could be improved by implementing advanced power management strategies, such as deep-sleep modes, event-driven wake-up mechanisms. This would make the solution more energy-efficient for outdoor deployments.
    \end{indt}

    \begin{indt}{\subsection{Use of a Magnetometer Instead of an Ultrasonic Sensor}}
        The current system relies on an ultrasonic sensor, which may detect objects other than vehicles and can be affected by environmental conditions. A magnetometer would provide more reliable vehicle detection by sensing disturbances in the Earth's magnetic field caused by large metallic objects, reducing false positives and improving accuracy.
    \end{indt}
...
\end{indt}
    ...
    \begin{indt}{\section{Annexes}} %{{{1
        \begin{itemize}[itemsep=0pt, leftmargin=\leftskip+12pt]
            ...
            \item Implementation repository (GitHub):
                \url{https://github.com/lasercata/IoT_parking_management}
        \end{itemize}
    \end{indt} %}}}1

    \begin{indt}{\section{Appendice}} %{{{1
        \listoftables
        \listoffigures
    \end{indt} %}}}1
